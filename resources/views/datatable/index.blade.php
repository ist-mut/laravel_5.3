...
<html>
<head>
<title>วิธีการใช้ framework datatables ใน Laravel</title>
<meta charset="utf-8">
		...
		<div class="table-responsive text-center">
			<table class="table table-borderless" id="table">
				...
				<tr class="item{{$item->id}}">
					...
					<td>{{$item->email}}</td>
					<td>@if(is_null($item->department_id)) - @else {{$item->department->department_name}} @endif</td>
					<td>{{$item->salary}}</td>
					<td><button class="edit-modal btn btn-info"
							data-info="{{$item->employee_id}},{{$item->first_name}},{{$item->last_name}},{{$item->email}},@if(!is_null($item->department_id)){{$item->department_id}} @endif,{{$item->salary}}">
							<span class="glyphicon glyphicon-edit"></span> Edit
						</button>
						<button class="delete-modal btn btn-danger"
							data-info="{{$item->employee_id}},{{$item->first_name}},{{$item->last_name}},{{$item->email}},{{$item->gender}},{{$item->salary}}">
...
<script>
	...
	function fillmodalData(details){
		...
		$('#email').val(details[3]);
		if(details[4]!='')
			$("#department option[value=" + details[4] + "]")
				.prop("selected",true);
            ...
            success: function(data) {
            	if (data.errors){
                    ...
                    if(data.errors.department) {
                    	$('.department_error').removeClass('hidden');
                        $('.department_error').text("Department must be a valid one !");
                    }
                ...
                $('.item' + data.id).replaceWith("<tr class='item" + data.id + "'><td>" +
                        ...
                        "</td><td>" + data.last_name + "</td><td>" + data.email + "</td><td>" +
                         data.department + "</td><td>" + data.country + "</td><td>" + data.salary +
                          "</td><td><button class='edit-modal btn btn-info' data-info='" + data.id+","+data.first_name+","+data.last_name+","+data.email+","+data.department+","+data.salary+"'><span class='glyphicon glyphicon-edit'></span> Edit</button> <button class='delete-modal btn btn-danger' data-info='" + data.id+","+data.first_name+","+data.last_name+","+data.email+","+data.gender+","+data.country+","+data.salary+"' ><span class='glyphicon glyphicon-trash'></span> Delete</button></td></tr>");
